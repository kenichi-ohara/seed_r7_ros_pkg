...
void StrokeConverter::Angle2Stroke (std::vector<int16_t>& _strokes, const std::vector<double> _angles)
{
  ...
  if(robot_model_ == "typeF"){
  ...
  else if(robot_model_ == "typeFFlESy"){
    ...
    _strokes[11] = scale * (rad2Deg * _angles[11] - 65.0) * 0.12;

    _strokes[14] = -scale * (rad2Deg * _angles[14] + 15) * 0.18;
    ...
    _strokes[26] = -scale * (rad2Deg * _angles[26] + 65.0) * 0.12;

    _strokes[29] = scale * (rad2Deg * _angles[29] - 15) * 0.18;
...
void StrokeConverter::Stroke2Angle (std::vector<double>& _angles, const std::vector<int16_t> _strokes)
{
  ...
  if(robot_model_ == "typeF"){
    ...
    _angles[11] = deg2Rad * (scale * _strokes[11] * 8.333 + 65.0);
    _angles[12] = 0;
    _angles[13] = 0;
    _angles[14] = -deg2Rad * (scale * _strokes[14] * 11.111 + 15.0);
    ...
    _angles[26] = -deg2Rad * (scale * _strokes[26] * 8.333 + 65.0);
    _angles[27] = 0;
    _angles[28] = 0;
    _angles[29] = deg2Rad * (scale * _strokes[29] * 5.556 + 15.0);
